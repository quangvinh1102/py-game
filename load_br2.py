...
for i in range(sl_item):
    ...
    Item_load = pygame.transform.scale(Item_load,(28, 28))
...
while running:
        ...
        for i in range(cao):
            for t in range(rong):
                ...
                if x_mouse > x_map and y_mouse > y_map and x_mouse < x_map + 32 and y_mouse < y_map + 32 and y_mouse <= 416:
                    if file_map[i][t] < 0 and file_map[i + 1][t] > -1 and file_item[i][t] != 3:
                        file_br2[i][t] = k
    for i in range(13):
    ...
    for i in range(sl_br2 + 1):
        ...
        if i < sl_br2:
            img_xt = pygame.transform.scale(Br2[a_map],(32, 32))
            screen.blit(img_xt, (b_map + 4, c_map + 4))
            
    if luu:
        file = open('file_map.txt','w')
        for i in range(cao):
            for t in range(rong):
                k_luu = file_map[i][t]
